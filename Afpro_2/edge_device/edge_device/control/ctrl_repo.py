...
class systemDevice:
    ...
    rated_power = 60000  # non zero for source and loads, irevelant for meters
    stptCurve = None
    device_id: int
    addr_map: dict = {}
    ctrl_map:dict = {}
    ...
    def __init__(self, devicetype, commtype, cfg,rated_power=60000) -> None:
    ...
    def encodeWrite(self, msg_json:dict):
        ...
        if msg_json["param"] == "active_power":
            ...
            #builder = BinaryPayloadBuilder(byteorder=Endian.LITTLE, wordorder=Endian.LITTLE)
                ...
                print("rated_power in encodeWrite : ",self.rated_power)
                print("power_value to set in encodeWrite : ",eval(msg_json['value']))
...
class operatingDetails:
    ...
    def export_limit_func(self):
        print("-------into export_limit_func-------")
        print("aggLoad in export_limit_func :",self.aggLoad)
        ...
        print("current pv power : ",self.aggPV)
        print(f"pv_stpt calculated : {self.pv_stpt}")
        print("-------export_limit_func end-------")
    ...
    def dg_pv_sync_func(self):
        ...
        #print("DG params : ",self.agg_pv_rated,self.aggPV,self.aggDG,self.dg_lim,self.pv_stpt,self)
...
def updateOperatingMode(mode_str, ref=0):
    print("-------into updateOperatingMode-------")
    ...
    if mode_str == "gen_limit":
        print("-------into gen_limit func of updateOperatingMode-------")
...
def getActiveControlMode():
    ...
    print("func is in getActiveControlMode",system_operating_details.controlFunc)
    ...
    if system_operating_details.controlFunc != None:
        ...
        for device in device_list:
            ...
            if device.device_type == deviceType.solar:
                print("solarType in runSysControlLoop")
                device.control_data.power_pct_stpt.value = system_operating_details.pv_stpt
                print("stpt_pct in runSysControlLoop : ",device.control_data.power_pct_stpt.value)
                data_msg = {"param" : "active_power","value":str(device.control_data.power_pct_stpt.value)}
                print("data_msg in runSysControlLoop : ",data_msg)
                device.encodeWrite(data_msg)
