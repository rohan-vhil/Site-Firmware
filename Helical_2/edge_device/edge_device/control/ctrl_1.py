...
class measuredData:
    ...
    prev_total_energy: float = 0.0

    def __init__(self, phase=1):
        self.phase = phase

    def initialize_from_map(self, addr_map, num_phases):
        all_map_keys = set()
        if 'map' in addr_map:
            for batch in addr_map["map"]:
                all_map_keys.update(addr_map["map"][batch]["data"].keys())

        for param_short, param_long in data_decode.items():
            if any(f"L{p + 1}_{param_long}" in all_map_keys for p in range(num_phases)):
                setattr(self, param_short, [dataModel() for _ in range(num_phases)])

        for param in agg_data:
            if param in all_map_keys:
                setattr(self, param, dataModel())
...
class systemDevice:
    ...
    def __init__(self, devicetype, commtype, cfg,rated_power=125000) -> None:
    ...
    def createMeasureMap(self,part_num):
        with open('modbus_mappings/mappings.json') as mapfile:
            ...
            self.measured_data.initialize_from_map(self.addr_map, self.num_phases)
    ...
    def createMeasureRegisterMap(self):
        ...
        for batch in self.addr_map["map"]:
            for p_idx in range(self.num_phases):
                phase_num = p_idx + 1
                for param_short, param_long in data_decode.items():
                    key = f"L{phase_num}_{param_long}"
                    if key in self.addr_map["map"][batch]["data"]:
                        if hasattr(self.measured_data, param_short):
                            param_list = getattr(self.measured_data, param_short)
                            if p_idx < len(param_list):
                                self.createMapForVar(param_list[p_idx], batch, i, key)
    ...
    def decodeData(self, data_set):
        ...
        for x in agg_data:
            if(hasattr(self.measured_data,x)):
                param = getattr(self.measured_data,x)
                param.data = data[param.block_num][param.offset : param.offset + param.size]
                param.getData(data)
...
def getAllData():
    ...
    for device in device_list:
        ...
        for param in agg_data:
            if hasattr(device.measured_data, param):
                current_value = getattr(device.measured_data, param).value
                if param == 'total_energy':
                    prev_value = device.measured_data.prev_total_energy
                    if current_value == 0:
                        continue
                    if current_value >= prev_value:
                        data[str(device.device_id)][param] = current_value
                        device.measured_data.prev_total_energy = current_value
                    else:
                        continue
                else:
                    data[str(device.device_id)][param] = current_value
