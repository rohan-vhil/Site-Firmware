...
class measuredData:
    ...
    total_voltage: Optional[dataModel]
    ...
    irradiance: Optional[dataModel]
    ambient_temperature: Optional[dataModel]
    internal_ambient_temperature: Optional[dataModel]
    module_temperature: Optional[dataModel]
    internal_module_temperature: Optional[dataModel]
    ...
    def __init__(self, phase: int = 1) -> None:
        ...
        self.internal_ambient_temperature = None
        self.module_temperature = None
        self.internal_module_temperature = None
...
class systemDevice:
    ...
    def __init__(self, devicetype: deviceType, commtype: commType, cfg: Dict[str, Any], rated_power=60000) -> None:
    ...
    def createMeasureRegisterMap(self):
        ...
        for batch_name, batch_content in self.addr_map["map"].items():
            ...
            if hasattr(self.measured_data, "today_energy"):
                 if "today_energy" not in current_agg_data_list :
                    current_agg_data_list.append("today_energy")

            for x in current_agg_data_list:
    ...
    def encodeWrite(self, msg_json:dict):
        ...
        if msg_json.get("param") == "active_power":
            ...
            try:
                ...
                print(f"target power : {target_power_value}")
                logging.info(f"target power to set : {target_power_value}")
            except ValueError:
                logging.error(f"Invalid numeric value for active_power: {value_str}")
                return

            if self.control_data.poweer_lt.model_present:
                print("-------into poweer_lt model-------")
                ...
                if self.rated_power == 0:
                    ...
                    return
                self.control_data.power_pct_stpt.value = int(target_power_value * 100.0 / self.rated_power)
                print(f"stpt_pct after dividing to rated_power : {self.control_data.power_pct_stpt.value}")
                if self.control_data.power_pct_stpt.value == 0:
                    self.control_data.power_pct_stpt.value = 1
                    print(f"updated stpt_pct : {self.control_data.power_pct_stpt.value}")
    ...
    def decodeData(self, data_set: Dict[str, Any]):
        ...
        if self.err_registers and hasattr(self.err_registers, 'errorDecode'):
            self.err_registers.errorDecode(read_data, self.device_id)

        params_to_decode = agg_data[:]
...
class operatingDetails:
    ...
    def export_limit_func(self):
        ...
        print(f"pv_stpt : {self.pv_stpt}")
        logging.info(f"pv_stpt : {self.pv_stpt}")
    ...
    for device in device_list:
        ...
        for param_name in parameters_to_check:
            ...
            if dm_instance is not None and isinstance(dm_instance, dataModel) and dm_instance.model_present:
                ...
                if param_name == "total_energy":
                    if value_to_check == 0:
                        continue
                    
                    previous_value = dm_instance.prev_correct_value
                    if value_to_check < previous_value:
                        logging.warning(f"Device {device.device_id}: total_energy rollover detected. Current: {value_to_check}, Previous: {previous_value}. Omitting value.")
                        continue
                    
                    dm_instance.prev_correct_value = value_to_check
        ...
        if device.device_type == deviceType.meter and \
           ...
           device.connected_to != str(deviceType.grid.value):
            if device.measured_data.total_power and device.measured_data.total_power.model_present:
                ...
                print(f"aggLoad : {system_operating_details.load}")
    ...
    if system_operating_details.controlFunc is not None:
        ...
        for device in device_list:
            ...
            if device.device_type == deviceType.solar:
                ...
                # power_val = 0.0
                # if device.rated_power != 0:
                #     power_val = system_operating_details.pv_stpt
                #     print(f"power_val in runSysControlLoop : {power_val}")
                power_val = device.control_data.power_pct_stpt.value
                print(f"power_val in runSysControlLoop : {power_val}")

                data_msg = {"param" : "active_power", "value": str(power_val), "device_id": str(device.device_id)}
                print(data_msg)
                logging.info(f"data_msg from runSysControlLoop : {data_msg}")
                device.encodeWrite(data_msg)
