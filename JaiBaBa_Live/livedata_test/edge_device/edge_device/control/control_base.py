    ...
    def encodeWrite(self, msg_json:dict):
        ...
        if msg_json["param"] == "active_power":
            if(self.control_data.poweer_lt.model_present):
                #print("write val",msg_json['value'],self.device_type)
                ...
                payload=decoded.build()
                print(payload,self.control_data.poweer_lt.value,self.control_data.poweer_lt.encode())
...
class operatingDetails:
    ...
    def export_limit_func(self):
        ...
        #print("Load : ",self.aggLoad,self.aggPV,self.aggBatt,self.limit_export,self.agg_pv_rated)
        #if(grid_power < 0):
