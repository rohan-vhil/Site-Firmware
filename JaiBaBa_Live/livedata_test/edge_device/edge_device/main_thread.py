...
from IEC_RTU import simple_server
...
def readDeviceList():
    ...
    if(not os.path.exists(project_device_path)):
        ...
        if(True):
            ...
            try:
                response = requests.post(url=url,json=project_cfg,verify=False,timeout=1)
...
def getData():
    ...
    while( not os.path.exists(path_config.path_cfg.base_path + "../submodules/RpiBackend/app/json_files/project_devices.json")):
        install_file = False
        print("no file")
        pass
    install_file = True
    ...
    while(install_file):
        ...
        if(ctrl.system_operating_details.live_data):
            livedata.livdataHandler(ctrl.getAllData())
            read_period = 1
...
if __name__ == "__main__":
    ...
    #simple_server.server = simple_server.c104.Server()
    simple_server.startIECServer()
        
    #getData()
    t1 = threading.Thread(target=getData)
    t2 = threading.Thread(target=rpthndler.data_handler.runDataLoop)
    tmqtt = threading.Thread(target=subscribe.start_subscriber)
    tiec = threading.Thread(target = simple_server.runServer,args=[rpthndler.data_handler.avg_data])
