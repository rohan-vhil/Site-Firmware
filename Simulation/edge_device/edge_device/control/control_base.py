...
#     def __init__(self, devicetype, commtype, cfg,rated_power=3800) -> None:
...
#         if(data_json['param'] == "total_power"):
...
#             data = {"AC total_powerSetPct": device.control_data.setpt}
...
#                 data_msg = {"param" : "total_power","value":str(power)}
...
#                 #data = {"AC total_powerPct": device.control_data.setpt}
#                 #device.writeDataToRegisters(data)

#             if device.device_type == deviceType.solar:
#                 device.control_data.power_pct_stpt.value = system_operating_details.pv_stpt
#                 data_msg = {"param" : "total_power","value":str(device.control_data.power_pct_stpt.value)}
#                 print(data_msg)
#                 device.encodeWrite(data_msg)
    ...
    def encodeWrite(self, msg_json:dict):
        if msg_json["param"] == "total_power":
            if(self.control_data.poweer_lt.model_present):
                ...
                self.control_data.power_pct_stpt.value = eval(msg_json['value'])*100/system_operating_details.agg_pv_rated 
