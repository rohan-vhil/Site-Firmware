...
# This is the solution to the "Too many open files" error when running as a service.
# Services (like systemd) often have a low default limit for open files (e.g., 1024).
# The rapid connect/disconnect cycle from the firmware can exhaust this limit.
# We increase it programmatically to prevent the script from crashing.
try:
...
async def update_dynamic_values(context, slave_id, part_num, role):
    ...
    while True:
        ...
        if role == "inverter":
            # 1. Calculate the potential power based on environmental conditions (the bell curve)
            potential_power = generate_bell_curve_value(hour_frac, rated_power)
            
            # 2. Assume the final power is the potential power, unless a limit is applied
            power_val = potential_power
            
            if ctrl_block:
                en = get_control_value(context, slave_id, ctrl_block, "power_limit_en")
                if en == 1:
                    # Firmware has enabled power limit control. Determine the maximum allowed power.
                    power_limit = rated_power # Default to max power if no specific limit is read
                    ...
                    # Check for percentage control
                        ...
                        if limit_pct is not None:
                            power_limit = int(rated_power * (limit_pct / 1000.0))
                    
                    # 3. The final power is the minimum of the potential power and the firmware limit.
                    power_val = min(potential_power, power_limit)

            GLOBAL_POWER_STATE["inverters"][slave_id] = power_val

        elif role == "load_meter":
            power_val = 100000 if 7 <= hour < 18 else 0
...
async def main():
    ...
    identity.MajorMinorRevision = "3.3" # Version Bump
