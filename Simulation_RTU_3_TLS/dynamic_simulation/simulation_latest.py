...
try:
    ...
    new_soft_limit = 4096
    if soft < new_soft_limit:
        resource.setrlimit(resource.RLIMIT_NOFILE, (new_soft_limit, hard))
        logger.info(f"Successfully increased file descriptor limit from {soft} to {new_soft_limit}.")
except (ValueError, OSError) as e:
    logger.warning(f"Could not increase file descriptor limit: {e}. This may cause issues if run as a service with high connection rates.")
...
# This dictionary will hold the current state (power, energy, etc.) for all devices, keyed by slave_id.
GLOBAL_DEVICE_STATE = {}
...
def generate_bell_curve_value(hour,hour_fraction, peak=60000):
...
async def update_dynamic_values(context, slave_id, part_num, role):
    ...
    # Get the persistent state for this device
    device_state = GLOBAL_DEVICE_STATE[slave_id]
    ...
    while True:
        ...
        if role == "inverter":
            potential_power = generate_bell_curve_value(hour,hour_frac, rated_power)
            total_power_val = potential_power
            ...
            if ctrl_block:
                ...
                if en == 1:
                    ...
                    if "power_limit_value" in ctrl_block["data"]:
                        ...
                        if limit_val is not None: power_limit = limit_val
                    elif "power_limit_pct" in ctrl_block["data"]:
                        limit_pct = get_control_value(context, slave_id, ctrl_block, "power_limit_pct")
                        if limit_pct is not None: power_limit = int(rated_power * (limit_pct / 1000.0))
                    total_power_val = min(potential_power, power_limit)

            # Update total energy: Increment by 1 if the inverter is working
            if total_power_val > 0:
                energy_increment_Wh = int(total_power_val * (60.0 / 3600.0))
                device_state['total_energy'] += energy_increment_Wh
            
            # Update the global state with the latest power
            device_state['power'] = total_power_val

        elif role == "load_meter":
            total_power_val = 100000 if 7 <= hour < 18 else 0
            if 15 <= hour < 15.5: total_power_val = 30000
            device_state['power'] = total_power_val
            if total_power_val > 0:
                energy_increment_Wh = int(total_power_val * (60.0 / 3600.0))
                device_state['total_energy'] += energy_increment_Wh
            ...
            # Calculate grid power based on the global state of all other devices
            total_inverter_power = sum(state['power'] for state in GLOBAL_DEVICE_STATE.values() if state['role'] == 'inverter')
            total_load_power = sum(state['power'] for state in GLOBAL_DEVICE_STATE.values() if state['role'] == 'load_meter')
            total_power_val = max(0, total_load_power - total_inverter_power)
            device_state['power'] = total_power_val
            if total_power_val > 0:
                energy_increment_Wh = int(total_power_val * (5.0 / 3600.0))
                device_state['total_energy'] += energy_increment_Wh

        # --- Populate Registers ---
        
        # NEW: Calculate Apparent and Reactive Power from Active Power
        assumed_power_factor = 0.95
        if total_power_val > 0:
            apparent_power_val = int(total_power_val / assumed_power_factor)
            # Ensure no math domain error if PF is slightly off
            if apparent_power_val**2 >= total_power_val**2:
                reactive_power_val = int(math.sqrt(apparent_power_val**2 - total_power_val**2))
            else:
                reactive_power_val = 0
        else:
            apparent_power_val = 0
            reactive_power_val = 0
            
        # NEW: Get the persistent total energy value
        total_energy_val = device_state.get('total_energy', 0)
        ...
        for block_name, block in reg_blocks.items():
            ...
            for name, meta in data_map.items():
                ...
                # Helper to write a 32-bit value to two registers
                def set_32bit_val(value):
                    high = value >> 16
                    low = value & 0xFFFF
                    if offset + 1 < len(regs):
                        regs[offset], regs[offset+1] = high, low

                if name == "total_power": # Assumed to be Active Power
                    if meta.get("format") == "decode_32bit_uint": set_32bit_val(total_power_val)
                elif name == "apparent_power": # NEW
                     if meta.get("format") == "decode_32bit_uint": set_32bit_val(apparent_power_val)
                elif name == "reactive_power": # NEW
                     if meta.get("format") == "decode_32bit_uint": set_32bit_val(reactive_power_val)
                elif name == "total_energy": # NEW
                     if meta.get("format") == "decode_32bit_uint": set_32bit_val(total_energy_val)
...
async def main():
    ...
    for dev in installer_cfg.get("device_list", []):
        ...
        # Initialize the global state for this device
        GLOBAL_DEVICE_STATE[slave_id] = {
            "role": role,
            "power": 0,
            "total_energy": 0  # This will persist across device "restarts"
        }
    ...
    identity.MajorMinorRevision = "3.4" # Version Bump
