...
def getval(point: c104.Point) -> None:
    """
    Synchronous callback to update a monitoring point before it is transmitted.

    C104 expects the updated value immediately, so this must remain non-async.
    """
    point.value = time.time()  # Simulate a measurement
...
async def main():
    ...
    point.on_before_auto_transmit(callable=getval)
    point.on_before_read(callable=getval)
