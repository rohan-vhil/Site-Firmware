# --- START: New Setup Block ---
import os
import sys

# Get the absolute path of the directory containing this script (main_thread.py)
# This will be /home/edge_device/edge_device/edge_device/
APP_DIRECTORY = os.path.dirname(os.path.abspath(__file__))

# Add this directory to the Python Path to ensure all local imports work
if APP_DIRECTORY not in sys.path:
    sys.path.insert(0, APP_DIRECTORY)

# Now that the path is set, we can import other modules
import path_config

# Set the global base_path that all other functions will use.
# This resolves the TypeError permanently.
...
'''def readDeviceList():
    ...
    # --- MODIFIED PATH ---
    # This now correctly points to ./installer_cfg/
    install_file_path = os.path.join(base_path, "installer_cfg", "installer_cfg.json")
    
    # You may also need to update these paths if they are still being used
    project_device_path = os.path.join(base_path, "project_devices.json") # Adjust if this file is elsewhere
    site_device_path = os.path.join(base_path, "site_devices.json") # Adjust if this file is elsewhere'''
...
def readDeviceList():
    ...
    site_device_path = path_config.path_cfg.base_path + "../submodules/RpiBackend/app/json_files/site_devices.json"
    ...
    if(not os.path.exists(project_device_path)):
        ...
        #     #url = "http://vhileams-alb-1176575355.eu-north-1.elb.amazonaws.com/ui/customer/project/no-auth/create-project-device"
        #     response = requests.post(url=url,json=project_cfg,verify=False)
...
#
    ...
    for device in installer_cfg["device_list"]:
        ...
        elif(device['comm_type'] == 'gpio'):
            devicetype = ctrl.deviceType_l2e[device['device_type']]
            ctrl.device_list.append(io.ioDevice(devicetype, ctrl.commType.gpio))
    ...
    #with open("status_cfg.json") as status_file:
        ...
        #         try:
        #             site_response = requests.post("http://vhileams-alb-1176575355.eu-north-1.elb.amazonaws.com/ui/customer/project/create-project-device",json=site_device_cfg,verify=False)
